...
**Notes:**
- **SDK-first routing:** `get_pairs`, `get_tokens`, `get_environments`, `get_orderbook`, and `get_candles` route through `@dexalot/dexalot-sdk` (no REST re-implementation).
- **REST-only (no SDK method):** `get_deployed_contracts`, `get_app_settings`, `get_oldest_candle_ts`, `get_blacklisted_addresses` stay on the public TRADE_API mountpoint. See ARCHITECTURE.md §5 for the full SDK-vs-REST table.
