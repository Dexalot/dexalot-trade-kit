...
#   - the profile was saved with an encrypted_key
#   - the selected client config was written with the passphrase env + --read-only
#   - the private key and passphrase never appeared on screen (masking holds)
...
PASS = "qa-passphrase-1"

# (prompt marker, raw bytes to send). The clients step is the raw-mode checkbox:
# Enter accepts the pre-checked Claude Desktop.
steps = [
    ("Profile name", "qa\n"),
    ("Network (", "testnet\n"),
    ("leave blank to skip", KEY + "\n"),
    ("Encrypt the key", "Y\n"),
    ("Passphrase (hidden", PASS + "\n"),
    ("Confirm passphrase (hidden", PASS + "\n"),
...
if PASS in text:
    fails.append("PASSPHRASE appeared on screen")
...
if not os.path.exists(toml):
    ...
    if "encrypted_key" not in body:
        fails.append("profile missing encrypted_key")
...
if not os.path.exists(desktop):
    ...
    if not entry:
        ...
        if "DEXALOT_KEYSTORE_PASSWORD" not in (entry.get("env") or {}):
            fails.append("passphrase env not wired into client config")
...
if fails:
    ...
    for f in fails:
        print("  -", f)
    sys.exit(1)
print("PASS: config init wrote an encrypted profile, auto-registered Claude Desktop")
print("      (passphrase env + --read-only), and never leaked the key/passphrase.")
PY
